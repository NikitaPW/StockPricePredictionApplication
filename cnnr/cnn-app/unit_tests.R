...
test_that("data upload is working", {
  ...
  name <- "WIG20_1_MONTH_NEW.csv"
  data_frame <<- choose_data(name, path = "../train_data/")
  expect_false(is.null(dim(data_frame)))
})

test_that("data division is working", {
  
  division <- divide_data(data_frame, 10 , 5)
  
  expect_false(is.null(division))
  expect_equal(length(division), 4)
  expect_equal(length(division$Test), 10 + 5 - 1)
  expect_equal(length(division$Predict), 5)
  
})

test_that("data preperation is working", {
  data <<- prepare_data(data_frame, 5, 10)
  expect_false(is.null(data))
  expect_equal(length(data), 6)
  expect_equal(length(data$Test), 10 + 5 - 1)
  expect_equal(length(data$Predict), 5)
})

test_that("model initialization", {
  model <<- model_initialization(5)
